...
extern uint8_t col_mixer[4][4];
extern uint8_t col_unmixer[4][4];
...
void aes_encrypt_pcbc(uint8_t *ptext, uint8_t key[16], uint8_t iv[16], uint32_t num_blocks, uint8_t *dest){
    uint8_t curr_iv[16];
    for(int i = 0; i < 16; i++){
        curr_iv[i] = iv[i];
    }
...
void aes_decrypt_pcbc(uint8_t *ctext, uint8_t key[16], uint8_t iv[16], uint32_t num_blocks, uint8_t *dest) {
    uint8_t curr_iv[16];
    for(int i = 0; i < 16; i++){
        curr_iv[i] = iv[i];
    }
...
void next_round_key(uint8_t round_key[4][4], uint8_t round_num) {
    ...
    for(int colnum = 0; colnum < 4; colnum++) {
        if(colnum == 0) {
            ...
            for(int rownum = 0; rownum < 4; rownum++){
                round_key[rownum][colnum] = round_key[rownum][colnum-1] ^ orig[rownum][colnum];
            }
        }
    }
}
...
void inv_mix_columns(uint8_t state[4][4]) {
    for(int colnum = 0; colnum < 4; colnum++){
        ...
        galois_mtx_mult(col_unmixer, col);
...
uint8_t rcon[] = {0, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};